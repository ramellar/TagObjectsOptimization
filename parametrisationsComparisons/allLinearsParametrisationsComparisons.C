void allLinearsParametrisationsComparisons() {
    ...
    std::vector<float> kappas = {1.0, 2.0, 10.0};
          ...
          for(Int_t h = 0 ; h < effMins.size() ; h ++)
            {
              ...
              linear->SetLineColor(kBlue-h);
              linear->SetLineWidth(1);

              linear->Draw("same");

              if(i==0 & j==0 & h==0) leg->AddEntry(linear, "Linear parametrisations", "L");
            }
        }
    }
          ...
          for(Int_t h = 0 ; h < effMins.size() ; h ++)
            {
              ...
              if(i==0 & j==0 & h==0) leg->AddEntry(marker, "Linear mid-efficiency point", "P");
            }
        }
    }
    ...
    TString pdfName = "PDFs/ALLlinears.pdf";
