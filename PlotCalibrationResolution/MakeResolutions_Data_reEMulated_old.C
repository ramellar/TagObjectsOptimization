...
void DrawHistogramsWithFits(std::vector<TH1F*> barrel_response_ptBins,
    ...
    TCanvas* canvas = new TCanvas("canvas", "Histograms with Fits", 1200, 1200);
    canvas->Divide(3, 4);
...
void MakeResolutions(TString file, int run_nmbr, TString era = "",  TString fit_option = "crystalball")
{
    ...
    inTree->SetBranchAddress("L1Tau_nTT",&nTT);
    ...
    inTree->SetBranchAddress("L1Tau_RawIEta",&in_IEta);
    ...
    TH1F* RawIEt = new TH1F("RawIEt","RawIEt", 14, 0, 140);
    ...
    TH1F* RawIEt_correct = new TH1F("RawIEt_correct","RawIEt", 14, 0, 140);
    ...
    for(UInt_t i = 0 ; i < inTree->GetEntries() ; ++i)
    {
        ...
        for(long unsigned int i = 0; i < ptBins.size()-1; ++i)
        {
            ...
            {
                if (l1tTauPt/tauPt < 0.5 or l1tTauPt/tauPt >= 1.5) { continue; }
                response_ptBins[i]->Fill(l1tTauPt/tauPt);
                // if (tauPt > 45 and tauPt < 50) {
                //    if (l1tTauPt/tauPt < 0.85) {
                //    cout << in_IEta << ',' << in_RawIEt << ',' << in_HasEM << ',' << in_IsMerged << ") => bad event" << endl;
                ...
                //    } else { 
                //    cout << in_IEta << ',' << in_RawIEt << ',' << in_HasEM << ',' << in_IsMerged << ") => good event" << endl;
                //    l1tau_histo_correct->Fill(l1tTauPt);
                //    RawIEt_correct->Fill(in_RawIEt);
