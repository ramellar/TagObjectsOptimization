...
void MakeResolutions_noCalib(TString file, int run_nmbr, TString era = "",  TString fit_option = "crystalball")
{
    ...
    inTree->SetBranchAddress("L1Tau_RawIEt",&l1tTauPt);
    ...
    TH1F* HasEM = new TH1F("HasEM","HasEM", 2, 0, 2);
    TH1F* IsMerged = new TH1F("IsMerged","IsMerged", 2, 0, 2);
    TH1F* l1tau_histo = new TH1F("l1tauPt","l1taupPt", 14, 0, 140);
    TH1F* HasEM_correct = new TH1F("HasEM_correct","HasEM", 2, 0, 2);
    TH1F* IsMerged_correct = new TH1F("IsMerged_correct","IsMerged", 2, 0, 2);
    TH1F* l1tau_histo_correct = new TH1F("l1tauPt_correct","l1tauPt", 14, 0, 140);
    ...
    for(UInt_t i = 0 ; i < inTree->GetEntries() ; ++i)
    {
        ...
        //if(l1tTauIso<1) { continue; } // choose isolated taus (l1tTauIso = 1)
        // if(l1tTauPt_float/tauPt<0.6)
        // { 
        //     pt->Fill(tauPt);
        //     eta->Fill(tauEta);
        //     l1tpt->Fill(l1tTauPt_float);
        //     continue;
        // }
        ...
        for(long unsigned int i = 0; i < ptBins.size()-1; ++i)
        {
            ...
            {
                response_ptBins[i]->Fill(l1tTauPt_float/tauPt);
                if (tauPt > 45 and tauPt < 50) {
                   if (l1tTauPt_float/tauPt < 0.85) {
                   l1tau_histo->Fill(l1tTauPt_float);
                   //tauPt_histo->Fill(tauPt);
                   HasEM->Fill(in_HasEM);
                   IsMerged->Fill(in_IsMerged);
                   } else { 
                   l1tau_histo_correct->Fill(l1tTauPt_float);
                   HasEM_correct->Fill(in_HasEM);
                   IsMerged_correct->Fill(in_IsMerged);
                   }
                }          
                if (abs(tauEta) < 1.305) { barrel_response_ptBins[i]->Fill(l1tTauPt_float/tauPt); }
                else if (abs(tauEta) < 2.1 and abs(tauEta) > 1.479) { endcap_response_ptBins[i]->Fill(l1tTauPt_float/tauPt); }
            }
        }

        for(long unsigned int i = 0; i < etaBins.size()-1; ++i)
        {
            if(abs(tauEta) > etaBins[i] and abs(tauEta) < etaBins[i+1]) { absEta_response_ptBins[i]->Fill(l1tTauPt_float/tauPt); }

            if(tauEta > etaBins[i] and tauEta < etaBins[i+1]) { plusEta_response_ptBins[i]->Fill(l1tTauPt_float/tauPt); }
            else if (tauEta < -etaBins[i] and tauEta > -etaBins[i+1]) { minusEta_response_ptBins[i]->Fill(l1tTauPt_float/tauPt); }
        }
        ...
        for(long unsigned int i = 0; i < ptBins.size()-1; ++i)
        {
            for(long unsigned int j = 0; j < etaBins.size()-1; ++j)
            {
                ...
                {
                    pt_resp_PtEtaBin[k]->Fill(l1tTauPt_float/tauPt);
                }
    ...
    TFile* fileout = new TFile("ROOTs/ROOTs_Run3_2025/resolutions_of_"+run_nmbr_str+"_reEmulated_no_calibration.root","RECREATE");
    ...
    l1tau_histo->Write();
    HasEM_correct->Write();
    IsMerged_correct->Write();    
    l1tau_histo_correct->Write();
    for(long unsigned int i = 0; i < ptBins.size()-1; ++i)
    {
