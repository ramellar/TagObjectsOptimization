...
#    
#    ax.errorbar(x_etaResp_barrel, y_etaResp_barrel, xerr=x_err_etaResp_barrel, yerr=y_err_etaResp_barrel, ls='None', label=barrel_label+' - Unpacked 2018 - Isolation', lw=2, marker='o', color=cmap(1))
#    ##            [ mean, sigma, alphaL,    nL, alphaR,    nR,  norm]
...
#    fig, ax = plt.subplots(figsize=(10,10))
#
#    ax.errorbar(x_etaResp_endcap, y_etaResp_endcap, xerr=x_err_etaResp_endcap, yerr=y_err_etaResp_endcap, ls='None', label=endcap_label+' - Unpacked 2022 - Isolation $p_{T}^{	au}$>0', lw=2, marker='s', color=cmap(1))
#    ##            [ mean, sigma, alphaL,    nL, alphaR,    nR,  norm]
#    p0 =          [   1.,   0.2,     1.,    1.,     1.,    1.,  0.1]
#    param_bounds=([ -10.,   0. ,     0.,    0.,     0.,    0.,  0. ],
#                  [  10.,   1. ,    10.,   25.,    10.,   25.,  1. ])
...
#    plt.close()
    ...
    ax.errorbar(x_etaResp_barrel, y_etaResp_barrel, xerr=x_err_etaResp_barrel, yerr=y_err_etaResp_barrel, ls='None', label=barrel_label, lw=2, marker='s', color=cmap(0))
    ##            [ mean, sigma, alphaL,    nL, alphaR,    nR,  norm]
...
#    barrel_label = r'$Barrel\ |\eta^{\tau, offline}|<1.305$'
#    endcap_label = r'$Endcaps\ 1.479<|\eta^{\tau, offline}|<2.1$'
#    barrel_label = r'$Inclusive\ |\eta^{\tau, offline}|<2.1$'
#    plot_name = 'responses/tau_eta_'+options.tag
#    if options.inclusive: plot_name = plot_name[:-4] + '_inclusive'
#
#    fig, ax = plt.subplots(figsize=(10,10))
#    
#    ax.errorbar(x_ptResol_barrel, y_ptResol_barrel, xerr=x_err_ptResol_barrel, yerr=y_err_ptResol_barrel, ls='None', label=barrel_label+' - Unpacked 2022 - Isolation $p_{T}^{	au}$>0', lw=2, marker='o', color=cmap(1))
...
#    fig, ax = plt.subplots(figsize=(10,10))
#
#    ax.errorbar(x_ptResol_endcap, y_ptResol_endcap, xerr=x_err_ptResol_endcap, yerr=y_err_ptResol_endcap, ls='None', label=endcap_label+' - Unpacked 2022 - Isolation $p_{T}^{	au}$>0', lw=2, marker='s', color=cmap(1))
...
#    plt.close()
    ...
    # PLOT PHI RESPONSE
    x_lim = (-0.25,0.25)
    y_lim = (min(y_phiResp_inclusive[9], y_phiResp_endcap[9]) * 0.6, max(max(y_phiResp_inclusive), max(y_phiResp_endcap)) * 1.4)
    ...
    ax.errorbar(x_phiResp_barrel, y_phiResp_barrel, xerr=x_err_phiResp_barrel, yerr=y_err_phiResp_barrel, ls='None', label=barrel_label, lw=2, marker='s', color=cmap(0))
    ##            [ mean, sigma, alphaL,    nL, alphaR,    nR,  norm]
    p0 =          [   0.,   0.2,     1.,    1.,     1.,    1.,  0.1]
    param_bounds=([ -10.,   0. ,     0.,    0.,     0.,    0.,  0. ],
                  [  10.,   1. ,    10.,   25.,    10.,   25.,  1. ])
