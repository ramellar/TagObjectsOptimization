...
if __name__ == "__main__" :
    ...
    #inFile2 = ROOT.TFile('/home/llr/cms/mchiusi/Run3preparation/Run3preparation_2023/CMSSW_11_0_2/src/TauObjectsOptimization/PlotTurnOns/ROOTs/ROOTs_2023/efficiencies_of_Run2022G_unpacked.root')
    
    inFile = ROOT.TFile('/home/llr/cms/mchiusi/Run3preparation/Run3preparation_2023/CMSSW_11_0_2/src/TauObjectsOptimization/PlotTurnOns/ROOTs/ROOTs_2023/efficiencies_of_Runall_2022_unpacked.root');
    inFile2 = ROOT.TFile('/home/llr/cms/mchiusi/Run3preparation/Run3preparation_2023/CMSSW_11_0_2/src/TauObjectsOptimization/PlotTurnOns/ROOTs/ROOTs_2023/efficiencies_of_Runhadd_2023_all_eras_before367607_unpacked.root')
    inFile3 = ROOT.TFile('/home/llr/cms/mchiusi/Run3preparation/Run3preparation_2023/CMSSW_11_0_2/src/TauObjectsOptimization/PlotTurnOns/ROOTs/ROOTs_2023/efficiencies_of_Runhadd_2023_all_eras_after367607_unpacked.root')
    ...
    if iso_string:
        ...
        #label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - LUT 2022' % (thr)
        label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2022' % (thr)
        #label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023 Unpacked' % (thr) # 2023B
    else:
        eff_TGraph = inFile.Get('divide_ptProgressionAt'+thr_string+'_noIso_by_pt')
        label = r'$E_{T}^{\tau, L1} > %i$ GeV - 2022' % (thr)
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV - LUT 2022' % (thr)
    ...
    ax.errorbar(x, y, xerr=x_err,yerr=[y_errD, y_errU], ls='None', label=label, lw=2, marker=marker, color='black', zorder=zorder)
    zorder = zorder + 1

    ##vectCBconvATAN
    #             [   mean, sigma, alpha,  n,    norm, xturn,   p, width]
    p0 =          [thr    ,    7.,   3. , 30.,   0.95,   10., 0.8,  60.]
    param_bounds=([thr-10.,    5.,   0.1,  10.,   0.9 ,  0., 0.2,   50.],
                  [thr+10.,    10,   10. , 100.,    1,  30., 1. ,   100.])
    #param_bounds=([thr-10.,    1.,   0.1,  10.,   0.9 ,    0., 0.2,  10.],
    #              [thr+10.,   10.,  10. , 100.,   1.  ,  110., 1. ,  80.])
    print(x)
    print(x[:10] + x[11:])
    popt, pcov = curve_fit(vectCBconvATAN, x[:9] + x[11:], y[:9] + y[11:], p0, maxfev=5000, bounds=param_bounds)

    print(popt)

    ax.plot(plot_x, vectCBconvATAN(plot_x, *popt), '-', label='_', lw=2, color='black', zorder=0)
   #      
   #  imap+=1
    zorder = zorder + 1
    ...
    if iso_string:
        eff_TGraph = inFile2.Get('divide_ptProgressionAt34_Iso_by_pt')
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - LUT 2023' % (thr)
        #label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023 Unpacked' % (thr) # 2023B
        label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023 before' % (34)
    else:
        eff_TGraph = inFile2.Get('divide_ptProgressionAt32_noIso_by_pt')
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV - LUT 2023' % (thr)
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV - 2023 Unpacked' % (thr)
        label = r'$E_{T}^{\tau, L1} > %i$ GeV - 2022' % (32)
    ...
    ##vectCBconvATAN
    #             [   mean, sigma, alpha,    n,     norm, xturn,   p, width]
    p0 =          [thr    ,    2.,   3. ,  30.,     0.95,   10., 0.8,   50.]
    param_bounds=([thr-10.,    1.,   0.1,  10.,     0.9 ,    0., 0.2,   40.],
                  [thr+10.,    10,   10. , 100.,       1,  40., 1. ,   60.])
    #param_bounds=([thr-10.,    1.,   0.1,  10.,   0.9 ,    0., 0.2,  40.],
    #              [thr+10.,   6.,  10. , 100.,   1.  ,  100., 1. ,  70.])
    #param_bounds=([thr-10.,    1.,   0.1,  10.,     0.9 ,    0., 0.2,   40.],
    #              [thr+10.,    3.5,   10. , 100.,       1,  100., 1. ,   60.])
    ...
    if iso_string:
        eff_TGraph = inFile3.Get('divide_ptProgressionAt34_Iso_by_pt')
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - LUT 2023' % (thr)
        #label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023 Unpacked' % (thr) # 2023B
        label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023 after' % (34)
    else:
        eff_TGraph = inFile3.Get('divide_ptProgressionAt32_noIso_by_pt')
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV - LUT 2023' % (thr)
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV - 2023 Unpacked' % (thr)
        label = r'$E_{T}^{\tau, L1} > %i$ GeV - 2022' % (32)
