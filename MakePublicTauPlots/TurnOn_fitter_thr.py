...
if __name__ == "__main__" :
    ...
    inFile = ROOT.TFile('/home/llr/cms/mchiusi/Run3preparation/Run3preparation_2023/CMSSW_11_0_2/src/TauObjectsOptimization/PlotTurnOns/ROOTs/ROOTs_2023/efficiencies_of_Run2023_all_eras_unpacked.root');
    ...
    if iso_string:
        ...
        label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023' % (thr)
    ...
    if iso_string:
        eff_TGraph = inFile.Get('divide_ptProgressionAt34_Iso_by_pt')
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - LUT 2023' % (thr)
        #label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023 Unpacked' % (thr) # 2023B
        label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023' % (34)
    else:
        eff_TGraph = inFile.Get('divide_ptProgressionAt34_noIso_by_pt')
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV - LUT 2023' % (thr)
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV - 2023 Unpacked' % (thr)
        label = r'$E_{T}^{\tau, L1} > %i$ GeV' % (34)
    ...
    if iso_string:
        eff_TGraph = inFile.Get('divide_ptProgressionAt38_Iso_by_pt')
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - LUT 2023' % (thr)
        #label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023 Unpacked' % (thr) # 2023B
        label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023' % (38)
    else:
        eff_TGraph = inFile.Get('divide_ptProgressionAt38_noIso_by_pt')
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV - LUT 2023' % (thr)
        # label = r'$E_{T}^{\tau, L1} > %i$ GeV - 2023 Unpacked' % (thr)
        label = r'$E_{T}^{\tau, L1} > %i$ GeV' % (38)
    ...
    inFile2 = ROOT.TFile('/home/llr/cms/mchiusi/Run3preparation/Run3preparation_2023/CMSSW_11_0_2/src/TauObjectsOptimization/PlotTurnOns/ROOTs/ROOTs_2023/efficiencies_of_Runall_2022_unpacked.root');

    if iso_string:
        eff_TGraph = inFile2.Get('divide_ptProgressionAt34_Iso_by_pt')
        #label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - LUT 2022' % (thr)
        label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2022' % (34)
        #label = r'$E_{T}^{\tau, L1} > %i$ GeV & Isolation - 2023 Unpacked' % (thr) # 2023B
    else:
