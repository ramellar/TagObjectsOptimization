...
def plot_pt_scale(inFile, label, color, ax, bin_tree='pt_response_ptInclusive'):
    x_lim = (0.,2.)
    x_label = r'$E_{T}^{\tau, L1}/p_{T}^{\tau, gen}$'
    ...
    ax.axvline(x=fit_mean+variance, color=cmap(18), linestyle='--', label=f'variance:$\pm${variance:.2f}')
...
def plot_pt_resolution(inFile, label, color, ax, bin_tree='pt_resol_fctPt', observable="pt"):
    barrel_label = r'$Barrel\ |\eta^{\tau, gen}|<1.305$'
    endcap_label = r'$Endcaps\ 1.479<|\eta^{\tau, gen}|<2.1$'
    inclusive_label = r'$Inclusive\ |\eta^{\tau, gen}|<2.1$'
    # inclusive_label = ''
    ...
    if observable == "eta":
        inclusive_label = r'Inclusive $p_t^{\tau,gen} < 30 $ GeV:'
        ax.errorbar(x_ptResol_inclusive, y_ptResol_inclusive, xerr=x_err_ptResol_inclusive, yerr=y_err_ptResol_inclusive, 
                ls='None', label= inclusive_label + "\n" +label, lw=2, marker='o', color=cmap(color))
...
def create_and_save_plot(single_bin, label, in_file, plot_tag):
    ...
    plot_pt_scale(in_file, label, 2, ax, single_bin)
    # mean, variance=plot_pt_scale(in_file, label, 0, ax, single_bin)
    plot_name = f'responses/gen_studies/tau_pt_scale_{plot_tag}_{single_bin}'
...
''' python3 Response_fitter_gen.py --inFile1 resolutions_of_Rungen_matched_MC25_unpacked.root --inFile2 resolutions_of_RunMC25_unpacked.root --tag matched_vs_nonmatched_MC25 '''
''' python3 Response_fitter_gen.py --inFile1 resolutions_of_Rungen_matched_MC25_unpacked.root --inFile2 "" --tag matched_vs_nonmatched_MC25 '''
''' python3 Response_fitter_gen.py --inFile1 resolutions_of_Rungen_matched_MC25_unpacked.root --inFile2 resolutions_of_RunMC25_unpacked.root --tag matched_vs_nonmatched_MC25 '''
...
''' python3 Response_fitter_gen.py --inFile1 resolutions_of_Runl1_to_gen_matching_MC25_unpacked.root --inFile2 "" --tag MC25_L1_to_gen_matching '''
...
if __name__ == "__main__" :
    ...
    main_folder = '/home/llr/cms/amella/Plotting_efficiency/CMSSW_13_2_0_pre3/src/HiggsAnalysis/TagObjectsOptimization/PlotCalibrationResolution/ROOTs/ROOTs_2025/gen_studies/'
    ...
    plot_name = 'responses/gen_studies/tau_pt_scale_'+options.tag
    print(plot_name+'.png') 
    ...
    if options.bins:
        ...
        for single_bin in single_eta_bins:
            # label_1_bins = "Run2025W-MC-MINIAOD\n" + \
            #     "caloparams 2023\n" + \
            #    f"{single_bin.split('pt_resp_')[-1]}"
            label_1_bins = "MC25W-MINIAOD\n" + \
               f"{single_bin.split('pt_resp_')[-1]}"
        ...
        # plt.xlabel(r'$p_t^{\tau, gen}$')
        # plt.ylabel(r"$\langle E_{T}^{\tau, L1}/p_{T}^{\tau, gen}\rangle$")
        # plt.legend()
        # plt.grid(True, linestyle=':', alpha=0.6)
        # for xtick in ax.xaxis.get_major_ticks():
        #     xtick.set_pad(10)
        # mplhep.cms.label('Preliminary', data=True, rlabel=r'13.6 TeV')
        # plot_name = f'responses/gen_studies/tau_mean_pt_scale_{options.tag}'
        # plt.savefig(f'{plot_name}.pdf')
        # plt.savefig(f'{plot_name}.png')
        # plt.close()
        ...
        #     xtick.set_pad(10)
        # mplhep.cms.label('Preliminary', data=True, rlabel=r'13.6 TeV')
        # plot_name = f'responses/gen_studies/tau_mean_eta_scale_{options.tag}'
        # plt.savefig(f'{plot_name}.pdf')
        # plt.savefig(f'{plot_name}.png')
        # plt.close()
        ...
        if options.inclusive:
            plot_name = f'responses/gen_studies/tau_pt_scale_{options.tag}_{single_bin}_inclusive'
            plt.savefig(f'{plot_name}.pdf')
            plt.savefig(f'{plot_name}.png')
            plt.close()
    ...
    plot_pt_resolution(inFile1, label1, 0, ax1, "pt_resol_fctPt")
    # plot_pt_resolution(inFile2, label2, 1, ax1 ,"pt_resol_fctPt") 
    plot_name1 = 'responses/gen_studies/tau_pt_resolution_' + options.tag + '_fctPt'
    ...
    plot_name2 = 'responses/gen_studies/tau_pt_resolution_' + options.tag + '_fctEta'
