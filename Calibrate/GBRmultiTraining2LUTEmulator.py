...
class GBRmultiTraining2LUTEmulator:
    def __init__(self, numberOfForests, forestsDemarcation):
        self.name = ""
        self.Nforests = numberOfForests
        self.forestsDemarcationCompressedE = forestsDemarcation
        self.inputFileName = "regression.root"
        if self.Nforests == 1:
            self.inputFile = None
            self.forest = ROOT.MakeNullPointer( "GBRForest" ) 
        elif self.Nforests == 2:
            self.inputFile0 = None
            self.inputFile1 = None
            self.forest0 = ROOT.MakeNullPointer( "GBRForest" ) 
            self.forest1 = ROOT.MakeNullPointer( "GBRForest" )
        else:
            print "ERROR: number of forests requested not supported"
            exit()
    ...
    def retrieveForests(self):
        ...
        if self.Nforests != len(self.inputFileName.split()): print "ERROR: number of forests requested and number of files passed do not match"; exit();
        if self.Nforests == 1:
            ...
            print "inputFileName0", self.inputFileName.split()[0]
            print "inputFileName1", self.inputFileName.split()[1]
            files = self.inputFileName.split()
            self.inputFile0 = ROOT.TFile.Open(files[0])
            self.inputFile1 = ROOT.TFile.Open(files[1])
            self.inputFile0.GetObject(self.forestName, self.forest0)
            self.inputFile1.GetObject(self.forestName, self.forest1)

            varlist = ROOT.MakeNullPointer( ROOT.vector("string") )
            self.inputFile0.GetObject("varlistEB", varlist)
        else:
            print "ERROR: number of forests requested not supported"
            exit()
    ...
    def createLUT(self, header=""):
        ...
        with open(self.outputFileName, 'w') as output:
            ...
            address = 0
            if header=="":
                header = """# Calibration vs |ieta|,ET,hasEM,isMerged Derived from 13 TeV data (RunD) using mu+tau Tag & Probe selection, with semi-parametric regression
        ...
        with open(self.outputFileName, 'w') as output:
            ...
            for inputs in product(*inputPoints):
                ...
                if self.Nforests == 1:
                    ...
                    if value2 <= self.forestsDemarcationCompressedE: resp = self.forest0.GetResponse(array.array('f',inputs))
                    else: resp = self.forest1.GetResponse(array.array('f',inputs))
                else:
                    print "ERROR: number of forests requested not supported"
                    exit()
