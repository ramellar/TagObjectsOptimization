...
// void ApplyCalibrationZeroBias(TString InputFileName = "Ntuple_ZeroBias_With2017Layer1_ShapeVeto_20_02_17.root")
{
  ...
  TString OutputFileName = "/data_CMS/cms/motta/Run3preparation/2022_01_28_optimizationV6_calibThr"+intgr+"p"+decim+"/EphemeralZeroBias_2018D_Run"+run_str+"_CALIBRATED.root";

  // TFile f_2DShapeVetos("ShapeVeto/TwoDShapeVetos.root","READ");
  // TH2I* TwoDShapeVetos = (TH2I*)f_2DShapeVetos.Get("TwoDShapeVetos");

  // TFile f_shapes("/home/llr/cms/cadamuro/Tau_ShapeVeto/CMSSW_8_0_10/src/L1Trigger/L1TNtuples/ShapeVeto/shapes_ZeroBias.root");
  ...
  // TH2F* shapes_vs_pt_isolated = new TH2F("shapes_vs_pt_isolated","shapes_vs_pt_isolated",128,0,128,NbinsIEt2-1,hardcodedIetBins2double);
  // shapes_vs_pt_isolated->Clear();  
  ...
  TFile f_histos("/home/llr/cms/motta/Run3preparation/CMSSW_11_0_2/src/TauObjectsOptimization/Calibrate/corrections/corrections_Trigger_Stage2_Run3_MC_VBFHToTauTau_M125_compressedieta_compressediet_hasEM_isMerged_optimizationV6.root","READ");
  TH3F* h_LUT_isMerged0 = (TH3F*)f_histos.Get("LUT_isMerged0_GBRFullLikelihood_Trigger_Stage2_Run3_MC_VBFHToTauTau_M125_compressedieta_compressediet_hasEM_isMerged_optimizationV6");
  TH3F* h_LUT_isMerged1 = (TH3F*)f_histos.Get("LUT_isMerged1_GBRFullLikelihood_Trigger_Stage2_Run3_MC_VBFHToTauTau_M125_compressedieta_compressediet_hasEM_isMerged_optimizationV6"); 
  ...
  // vector<Bool_t>  *out_L1Tau_PassShapeVeto_0p6_10_50 = 0;
  ...
  // outTree.Branch("L1Tau_PassShapeVeto_0p6_10_50",&out_L1Tau_PassShapeVeto_0p6_10_50);
  ...
  //for(UInt_t i = 0 ; i < 100000 ; ++i)
    {
      ...
      // out_L1Tau_PassShapeVeto_0p6_10_50->clear();

      //      cout<<"here before L1 tau loop"<<endl;
      ...
      for(UInt_t iL1Tau = 0 ; iL1Tau < l1tEmuPt->size() ; ++iL1Tau)
	{
	  ...
	  // short int symmShape0 = getSymmShape (l1tEmuQual->at(iL1Tau), remap);
	  // if(TwoDShapeVetos->GetBinContent(symmShape0,out_compressedE->at(iL1Tau))==1)
	  //   {
	  //     out_L1Tau_PassShapeVeto_0p6_10_50->push_back(true);
	  //   }
	  // else out_L1Tau_PassShapeVeto_0p6_10_50->push_back(false);
	  ...
	  // shapes_vs_pt->Fill(symmShape,out_l1tEmuRawEt->at(iL1Tau));
	  
	  Int_t Cut_L1Tau_Iso_Option22  = histosIsolation["LUT_Progression_22"]->GetBinContent(out_compressedieta->at(iL1Tau)+1,out_compressedE->at(iL1Tau)+1,out_compressednTT->at(iL1Tau)+1);
	  // if(out_l1tEmuIsoEt->at(iL1Tau)<=Cut_L1Tau_Iso_Option22) shapes_vs_pt_isolated->Fill(symmShape,out_l1tEmuRawEt->at(iL1Tau));
	  

	  isolation_vs_pt->Fill(l1tEmuIsoEt->at(iL1Tau),out_l1tEmuRawEt->at(iL1Tau));
	  // shapes_vs_isolation_vs_pt->Fill(symmShape,l1tEmuIsoEt->at(iL1Tau),out_l1tEmuRawEt->at(iL1Tau));

	  // shapes_vs_pt->Fill(symmShape,out_l1tEmuRawEt->at(iL1Tau)/2.);
	  // shapes_vs_eta->Fill(symmShape,out_l1tEmuTowerIEta->at(iL1Tau));
	  // if(out_l1tEmuRawEt->at(iL1Tau)/2.>=30.) shapes_1D->Fill(symmShape);

	  // out_L1Tau_PassShapeVeto->push_back(true);
	  //cout<<(out_l1tEmuQual->at(iL1Tau)&=127)<<endl;
	  histo_shape->Fill((out_l1tEmuQual->at(iL1Tau)&=127));
  ...
  // TFile f_out_shapes("background_shapes_forTraining_finner_binning.root","RECREATE");
  // shapes_vs_pt->Write("shapes_vs_pt");
  // shapes_vs_pt_isolated->Write("shapes_vs_pt_isolated");
