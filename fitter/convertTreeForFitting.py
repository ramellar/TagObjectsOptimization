...
fname = "/data_CMS/cms/motta/Run3preparation/SingleMuon__Run2018D-PromptReco-v2__MINIAOD__GoldenJSON__WMassCutTrue__ZMassCutTrue/SingleMuon__Run2018D-PromptReco-v2__MINIAOD__GoldenJSON__WMassCutTrue__ZMassCutTrue.root"
#pt = [20, 26, 30, 34]
pt = [20, 26, 30, 32, 34, 36]
...
briso   = [np.zeros(1, dtype=int) for x in range (0, len(pt))]
brnoiso = [np.zeros(1, dtype=int) for x in range (0, len(pt))]
bkgSubW = np.zeros(1, dtype=float)

hltPathTriggered_OS   = [np.zeros(1, dtype=int) for x in range (0, numberOfHLTTriggers+1)]
