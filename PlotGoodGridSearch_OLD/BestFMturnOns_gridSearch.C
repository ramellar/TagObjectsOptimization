...
void compare(int run, TString baseline="22", float limitTurnonThr=99, bool zoom=false, float calibThr = 1.7) {
    ...
    for (long unsigned int i = 0; i < tags.size(); ++i)
    {
        ...
        for (long unsigned int j = 0; j < opts.size(); ++j)
        {
            ...
            TFile* f = new TFile("/data_CMS/cms/motta/Run3preparation/2022_02_02_optimizationV7_calibThr"+intgr+"p"+decim+"/Run3_MC_VBFHToTauTau_M125_TURNONS_FIXEDRATE_Run"+run_str+"_gs_"+tag+"_"+opt+"_2022_02_02.root","READ");
    ...
    TString Filename = "plotsBestFMturnons_Run"+run_str+"/Comparison_TurnOn_Rate_Run"+run_str+"_newnTT_unpacked_optimizationV7gs_calibThr"+intgr+"p"+decim+"_betterThan"+baseline+"_thrLeq"+thrLim+".txt";
    ofstream file;
